...
const { request, elementText, elementExists, element, getResponse, validateResponseValues } = require('../lib/testHelpers');
...
describe('Share link exists', function() {
  ...
  it('exists', done => {
    request()
      ...
      .field('field1', 'field1')
      .end((err, res) => {
        expect(elementText(res.text, "footer button")).to.equal('Share');
        done(err);
      });
	});
