...
const Module = {
	...
	elementExists: function (html, selector) {
		const element = Module.document(html).querySelector(selector);
	...
	getResponse: function (method, query) {
		switch (method) {
			case 'get':
				return Module.request().get('/').query(query);
			case 'post':
				return Module.request().post('/').send(query);
		}
	},
	...
	validateResponseValues: function (method, query, cb) {
		Module.getResponse(method, query).end((err, res) => {
			...
			for (let key in query) {
				expect(Module.elementText(res.text, 'th.name')).to.equal(key + ':');
